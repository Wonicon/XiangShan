...
class RobImp(override val wrapper: Rob)(implicit p: Parameters, params: BackendParams) extends LazyModuleImp(wrapper)
  ...
  state_next := Mux(
    ...
    Mux(
      state === s_walk && walkFinished && rab.io.status.walkEnd && vtypeBuffer.io.status.walkEnd && mtypeBuffer.io.status.walkEnd, s_idle,
      state
    )
  )
  ...
  class CommitCondTrace extends Bundle {
    ...
    val commitCond = Bool()
    val commitCond_noAMUbackpressure = Bool()
    val needFlush = Bool()
    val b2bEnq = Bool()
  }
  ...
  for (i <- 0 until RobSize) {
    ...
    val commitCond_noAMUbackpressure =
      io.commits.isCommit && io.commits.commitValid.zip(deqSelOH).map(x => x._1 && x._2).reduce(_ || _)
    assert(PopCount(enqOH) < 2.U, s"robEntries$i enqOH is not one hot")
    ...
    when(commitCond_noAMUbackpressure) {
    ...
    val trace = Wire(new CommitCondTrace)
    trace.cycle := GTimer()
    trace.enqOH := enqOH
    ...
    trace.commitCond_noAMUbackpressure := commitCond_noAMUbackpressure
    trace.needFlush := needFlush
    trace.b2bEnq := enqOH.asUInt.orR && !io.redirect.valid
    commitCond_tbl.log(trace, commitCond || commitCond_noAMUbackpressure || trace.b2bEnq || needFlush,
                       "rob entry update", clock, reset)
  }
