...
class Rename(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper with HasPerfEvents {
  ...
  val io = IO(new Bundle() {
    ...
    val debug_mx_rat  = if (backendParams.debugEn) Some(Vec(3, Input(UInt(PhyRegIdxWidth.W)))) else None
  ...
  val needMxDest     = Wire(Vec(RenameWidth, Bool()))
  ...
  val walkNeedMxDest = WireDefault(VecInit(Seq.fill(RenameWidth)(false.B)))
  ...
  val mxSpecWen = Wire(Vec(RenameWidth, Bool()))
  ...
  for (i <- 0 until RenameWidth) {
    ...
    needMxDest(i) := io.in(i).valid && needDestReg(Reg_Mx, io.in(i).bits)
    if (i < RabCommitWidth) {
      ...
      walkNeedMxDest(i) := io.rabCommits.walkValid(i) && needDestRegWalk(Reg_Mx, io.rabCommits.info(i))
...
//    }
  ...
  for (i <- 1 until RenameWidth) {
    ...
    val mxCond = io.in(i).bits.srcType.map(_ === SrcType.mx)
    val target = io.in(i).bits.lsrc
    for ((((((cond1, ((condV0, condVl), condMx)), cond2), cond3), t), j) <- vecCond.zip(v0Cond.zip(vlCond).zip(mxCond)).zip(fpCond).zip(intCond).zip(target).zipWithIndex) {
      val destToSrc = io.in.take(i).zipWithIndex.map { case (in, j) =>
        val indexMatch = in.bits.ldest === t
        val writeMatch =  cond3 && needIntDest(j) || cond2 && needFpDest(j) || cond1 && needVecDest(j) || condMx && needMxDest(j)
  ...
  // other stall
  val otherStall = notRecStall && !intFlStall && !fpFlStall && !vecFlStall && !v0FlStall && !mxFlStall && !vlFlStall && !multiFlStall
  ...
  io.stallReason.in.backReason.bits := Mux(io.stallReason.out.backReason.valid, io.stallReason.out.backReason.bits,
    MuxCase(TopDownCounters.OtherCoreStall.id.U, Seq(
      ...
      mxFlStall     -> TopDownCounters.MxFlStall.id.U,
  ...
  for (i <- 0 until RabCommitWidth) {
    ...
    XSDebug(io.rabCommits.isWalk && io.rabCommits.walkValid(i), p"[#$i walk info] " +
      p"ldest:${info.ldest} rfWen:${info.rfWen} fpWen:${info.fpWen} vecWen:${info.vecWen} v0Wen:${info.v0Wen} vlWen:${info.vlWen} mxWen:${info.mxWen}")
  }
  ...
  XSPerfAccumulate("stall_cycle_matrix",   stallForMxFL)
