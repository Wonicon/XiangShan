...
class IssueQueueImp(override val wrapper: IssueQueue)(implicit p: Parameters, val params: IssueBlockParams)
  ...
  val mxWbBusyTableWrite = params.exuBlockParams.map { case x => Option.when(x.mxLatencyCertain)(Module(new FuBusyTableWrite(x.mxFuLatencyMap))) }
  val mxWbBusyTableRead = params.exuBlockParams.map { case x => Option.when(x.mxLatencyCertain)(Module(new FuBusyTableRead(x.mxFuLatencyMap))) }
  ...
  private def lastConnectFunc(exuInput: ExuInput, newInput: ExuInput): ExuInput = {
    ...
    if (newExuInput.mxWenCopy.nonEmpty && !lastExuInput.mxWenCopy.nonEmpty) {
      newExuInput.mxWenCopy.get.foreach(_ := lastExuInput.mxWen.get)
    }
  ...
  val mxWbBusyTableIn = io.wbBusyTableRead.map(_.mxWbBusyTable)
  ...
  val mxWbBusyTableOut = io.wbBusyTableWrite.map(_.mxWbBusyTable)
  ...
  val mxDeqRespSetOut = io.wbBusyTableWrite.map(_.mxDeqRespSet)
  ...
  val mxWbBusyTableMask = Wire(Vec(params.numDeq, UInt(params.numEntries.W)))
  ...
  entries.io match { case entriesIO: EntriesIO =>
    ...
    entriesIO.mxFromIntIsZero                                   := io.mxFromIntIsZero
    entriesIO.mxFromIntIsMxmax                                  := io.mxFromIntIsMxmax
    entriesIO.mxFromMfIsZero                                    := io.mxFromMfIsZero
    entriesIO.mxFromMfIsMxmax                                   := io.mxFromMfIsMxmax
  ...
  canIssueMergeAllBusy.zipWithIndex.foreach { case (merge, i) =>
    ...
    val mergeMxWbBusy = {
      if (mxWbBusyTableRead(i).nonEmpty) mergeVlWbBusy & (~mxWbBusyTableMask(i))
      else mergeVlWbBusy
    }
    merge := mergeMxWbBusy
  }
  ...
  mxWbBusyTableWrite.zip(mxWbBusyTableOut).zip(mxDeqRespSetOut).zipWithIndex.foreach { case (((busyTableWrite: Option[FuBusyTableWrite], busyTable: Option[UInt]), deqResp), i) =>
    if (busyTableWrite.nonEmpty) {
      val btwr = busyTableWrite.get
      val bt = busyTable.get
      val dq = deqResp.get
      btwr.io.in.deqResp := toBusyTableDeqResp(i)
      btwr.io.in.deqResp.valid := toBusyTableDeqResp(i).valid && deqBeforeDly(i).bits.common.mxWen.getOrElse(false.B)
      btwr.io.in.og0Resp := io.og0Resp(i)
      btwr.io.in.og1Resp := io.og1Resp(i)
      bt := btwr.io.out.fuBusyTable
      dq := btwr.io.out.deqRespSet
  ...
  mxWbBusyTableRead.zip(mxWbBusyTableIn).zipWithIndex.foreach { case ((busyTableRead: Option[FuBusyTableRead], busyTable: Option[UInt]), i) =>
    if (busyTableRead.nonEmpty) {
      val btrd = busyTableRead.get
      val bt = busyTable.get
      btrd.io.in.fuBusyTable :=bt
      btrd.io.in.fuTypeRegVec := fuTypeVec
      mxWbBusyTableMask(i) := btrd.io.out.fuBusyTableMask
    }
    else {
      mxWbBusyTableMask(i) := 0.U(params.numEntries.W)
    }
  }
  ...
  deqBeforeDly.zipWithIndex.foreach { case (deq, i) =>
    ...
    deq.bits.common.mxWen.foreach(_ := deqEntryVec(i).bits.payload.mxWen)
  ...
  io.wakeupToIQ.zipWithIndex.foreach { case (wakeup, i) =>
    ...
    if (wakeUpQueues(i).nonEmpty) {
      ...
      wakeup.bits.mxWen  := (if (wakeUpQueues(i).get.io.deq.bits.mxWen .nonEmpty) wakeUpQueues(i).get.io.deq.valid && wakeUpQueues(i).get.io.deq.bits.mxWen.get else false.B)
    }
    ...
    if (wakeUpQueues(i).nonEmpty && wakeup.bits.mxWenCopy.nonEmpty) {
      wakeup.bits.mxWenCopy.get := wakeUpQueues(i).get.io.deq.bits.mxWenCopy.get
    }
...
class IssueQueueMemAddrImp(override val wrapper: IssueQueue)(implicit p: Parameters, params: IssueBlockParams)
  ...
  io.wakeupToIQ.zip(params.exuBlockParams).zipWithIndex.foreach { case ((wakeup, param), i) =>
    ...
    if (param.hasLoadWakeupExu) {
      ...
      wakeup.bits.mxWen  := (if (params.writeMxRf)  GatedValidRegNext(uop.bits.mxWen && uop.fire) else false.B)
      ...
      wakeup.bits.mxWenCopy .foreach(_.foreach(_ := (if (params.writeMxRf)  GatedValidRegNext(uop.bits.mxWen  && uop.fire) else false.B)))
